//! GoalSummaryCell — 目标总结 Cell
//!
//! 绑定 Summarizer Agent，使用 InMemorySession。
//! 用户通过多轮对话确定目标，确认后 goal 持久化到 DB。
...
#[async_trait]
impl SessionCell for GoalSummaryCell {
    fn cell_id(&self) -> &str { &self.cell_id }
    fn workspace_id(&self) -> &str { &self.workspace_id }
    fn node_id(&self) -> &str { &self.node_id }
    fn cell_type(&self) -> CellType { CellType::GoalSummary }
    fn agent_type(&self) -> Option<AgentType> { Some(AgentType::Summarizer) }
    ...
    async fn add_message(&self, role: &str, content: &str) {
        let msg = Message {
            ...
            agent_type: AgentType::Summarizer.to_string(),
